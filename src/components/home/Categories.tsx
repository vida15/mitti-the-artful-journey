...
export function Categories() {
  return (
    <section className="relative px-6 py-32 md:px-10 md:py-48">
      <div className="pointer-events-none absolute inset-x-0 top-20 select-none text-center font-editorial italic leading-none" style={{ fontSize: "clamp(80px, 14vw, 180px)", color: "rgba(47,42,38,0.06)" }}>
        Collections
      </div>
      <p className="relative mb-16 text-center font-editorial italic" style={{ fontSize: "clamp(28px, 3vw, 48px)", color: "#2F2A26" }}>
        Browse by medium.
      </p>
      <div className="relative mx-auto grid max-w-7xl grid-cols-1 gap-4 md:grid-cols-12 md:grid-rows-7">
        {blocks.map((b) => (
          ...
          >
            ...
            />
            <div className="absolute inset-x-0 bottom-0 translate-y-full p-6 transition-transform duration-500 group-hover:translate-y-0" style={{ backgroundColor: "rgba(47,42,38,0.92)" }}>
              <p className="font-mono text-[10px] uppercase tracking-widest" style={{ color: "#B98B73" }}>
                Open ↗
              </p>
              <h3 className="font-editorial italic text-3xl" style={{ color: "#F7F3EE" }}>
                {b.name}
              </h3>
            </div>
          </Link>
