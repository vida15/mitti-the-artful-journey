    ...
    <div className="grid grid-cols-1 gap-12 px-6 py-16 md:grid-cols-[3fr_2fr] md:gap-16 md:px-10 md:py-24">
      <form onSubmit={submit} className="flex flex-col gap-8">
        ...
        <fieldset className="space-y-5">
          ...
          <input className={input} style={{ borderColor: "rgba(65,100,74,0.4)", color: "var(--color-bone)" }} placeholder="Email" type="email" required value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} />
        </fieldset>

        <fieldset className="space-y-5">
          <legend className="mb-3 font-mono text-[11px] uppercase tracking-widest" style={{ color: "var(--color-dust)" }}>Shipping</legend>
          <input className={input} style={{ borderColor: "rgba(65,100,74,0.4)", color: "var(--color-bone)" }} placeholder="Full name" required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} />
          <input className={input} style={{ borderColor: "rgba(65,100,74,0.4)", color: "var(--color-bone)" }} placeholder="Address" required value={form.line1} onChange={(e) => setForm({ ...form, line1: e.target.value })} />
          <div className="grid grid-cols-2 gap-5">
            <input className={input} style={{ borderColor: "rgba(65,100,74,0.4)", color: "var(--color-bone)" }} placeholder="City" required value={form.city} onChange={(e) => setForm({ ...form, city: e.target.value })} />
            <input className={input} style={{ borderColor: "rgba(65,100,74,0.4)", color: "var(--color-bone)" }} placeholder="PIN" required value={form.pincode} onChange={(e) => setForm({ ...form, pincode: e.target.value })} />
          </div>
          <input className={input} style={{ borderColor: "rgba(65,100,74,0.4)", color: "var(--color-bone)" }} placeholder="Country" required value={form.country} onChange={(e) => setForm({ ...form, country: e.target.value })} />
        </fieldset>
          ...
          <div className="grid grid-cols-2 gap-3">
            {(["upi", "cod"] as const).map((m) => (
              <button
                ...
                style={{
                  borderColor: method === m ? "var(--color-ember)" : "rgba(65,100,74,0.4)",
      ...
      <aside className="md:sticky md:top-[80px] md:self-start">
        <div className="border p-6" style={{ borderColor: "rgba(65,100,74,0.3)" }}>
          ...
          <div className="mt-6 flex items-end justify-between border-t pt-6" style={{ borderColor: "rgba(65,100,74,0.3)" }}>
