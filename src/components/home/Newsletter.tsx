...
export function Newsletter() {
  ...
  return (
    <section
      ...
      style={{ backgroundColor: flash ? "var(--color-ember)" : "#2F2A26" }}
    >
      <span
        className="pointer-events-none absolute inset-0 flex items-center justify-center font-display leading-none"
        style={{ fontSize: "clamp(120px, 22vw, 320px)", color: "rgba(247,243,238,0.05)" }}
      >
        STAY
      </span>
      <div className="relative w-full max-w-2xl text-center">
        {!done ? (
          <>
            <h2 className="font-editorial italic leading-tight" style={{ fontSize: "clamp(36px, 5vw, 56px)", color: "#F7F3EE", fontWeight: 400 }}>
            ...
            <form onSubmit={submit} className="mx-auto mt-12 flex max-w-md items-center gap-3">
              <input
                ...
                style={{ color: "#F7F3EE", borderColor: "rgba(247,243,238,0.4)" }}
              />
              <button type="submit" data-cursor="link" className="font-editorial italic text-3xl" style={{ color: "#B98B73" }}>
                →
              </button>
            </form>
          </>
          ...
          <motion.h2
            ...
            className="font-editorial italic" style={{ fontSize: "clamp(36px, 5vw, 56px)", color: "#F7F3EE" }}
          >
